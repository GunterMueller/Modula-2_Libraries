...
struct name *		np;
{
	...
	q = np->n_name;
	if (!(suff = suffix(q)))
/* 15.9.88 - RJN - Unix make behaves differently and on DOS you can't
		   begin a file with a "." */
#ifdef MSDOS || unix
		suff = np->n_name + strlen(np->n_name);
#else
		...
		for (dp = lp->l_dep; dp; dp = dp->d_next)
		{
			...
			if (sp->n_flag & N_TARG)
			{
				...
				if (!op->n_time)
					modtime(op);
				/* 15.9.88 - RJN 
				   Added the N_TARG check, because
				   an implicit dependent may not exist, 
				   but it still is a dependent if it is
				   in the list of explicit targets.
				*/
...
makerules()
{
	...
	/* 13.9.88 - RJN - Compatibility with other makes */
	setmacro("MAKE", "make");
