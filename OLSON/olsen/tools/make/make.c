...
#ifdef MSDOS
	/* 15.9.88 - RJN 
	 * To fix the problem of "dosh" not returning error
	 * codes.  The special file MAKESTOP can
	 * be created to get make to stop.   This is a
	 * hack of the worst kind, but so is MSDOS!  We
	 * always remove it before starting and if we 
	 * can remove it after the shell is done, there
	 * was an error.  This at least allows the user
	 * to create some batch files to do the dirty work.
	 */
	(void) unlink(MAKESTOP);
	number = system(string);
	if (unlink(MAKESTOP) == 0 && !number)	/* always delete the file */
		number = 86;
	return number;
#endif
#ifdef eon
...
int			level;
{
	...
	for (qdp = (struct depend *)0, lp = np->n_line; lp; lp = lp->l_next)
	{
		...
		{
			/* 15.9.88 - RJN
			   When recursing, must save the $< and $* macros,
			   because dependents may have different names
			   than the targets.  (Probably not the best fix.)
			 */
			if (implicitrule && starmacro == (char *)0)
			{
				tmp = getmacro("*");
				starmacro = malloc(strlen(tmp)+1);
				strcpy(starmacro, tmp);
				tmp = getmacro("<");
				ltmacro = malloc(strlen(tmp)+1);
				strcpy(ltmacro, tmp);
			}
		...
		{
			/* 15.9.88 - RJN - Gotta set'em everytime */
			if (starmacro)
			{
			    (void)setmacro("*", starmacro);
			    (void)setmacro("<", ltmacro);
			}
	...
	if (quest)
	{
		...
		t = np->n_time;
		time(&np->n_time);
		result = t < dtime;	/* RJN - fall through */
	}
	...
	/* 15.9.88 - RJN - Free it up.  C strings are as bad as Modula-2 */
	if ( starmacro )
	{
		free(starmacro);
		free(ltmacro);
