 ...
 *	- Added include and export features.
 * 	- Made "=" and ":" equivalent.  Previously, "=" had precedence.
...
FILE *			fd;
{
	...
	for(;;)
	{
		...
		/* 13.9.88 - RJN - 
		 * '=' was given higher priority over ':'.  Now
		 * there is no particular reason for this and it
		 * breaks the ${XXX:old=new} macro expansion for dependents.
		 * e.g. foobar: ${OBJ:.c=.o}.
		 */

		/* Only equal when both are zero! */
		while ((q = index(p, '=')) != (r = index(p, ':')))
		{
			/* min( r, q ) excluding 0 case */
			q = q < r ? ( q ? q : r ) : ( r ? r : q );

			/* Make sure no escaped characters */
			if ((p == q) || (q[-1] != '\\'))
				break;
			else
			{
				register char *		a;
				/*  Del \ chr; move rest back  */
				a = q - 1;
				p = q;
				while((*a++ = *q++) != 0)
				    ;
		...
		if (q == (char *)0)
		{
			/* RJN - 13.9.88 */
			/* 
			 * Check for include, export, and import declarations.
			 */
			q = str1;
		    	if ((p = gettok(&q)) != 0 && strcmp(p, "include") == 0) 
			{
				if (!(p = gettok(&q)))
					error("Include which file?");
				/*
				 * Open the include and recurse.  The "include"
				 * function does the file management and will
				 * call us recusively with the new "fd".
				 */
				include(p);
			}
			else if ( p && ( strcmp(p, "export") == 0 
				       || strcmp(p, "import") == 0 ) )
			{
				bool	ok = FALSE;
				if ( *p == 'i' ) func = import;
				...
				r = p;
				for(; (p = gettok(&q)) != 0; ok = TRUE)
					(*func)(p);
				if (!ok)
					error("%s which macros?", r);
			}
			else
