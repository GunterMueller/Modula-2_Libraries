	...
	;*	ends up deepest in the stack.  (There's another version	*)
	...
	;*			the compiler's default method.		*)
	;*								*)
	;*	Note: this version sets up task stacks in such a way	*)
	;*	that SS:FFFF is the last address in the stack.  This	*)
	;*	turns out to be unsuitable for use with TopSpeed	*)
	;*	because some TopSpeed library functions make		*)
	;*	the assumption that the stack segment is an expand-up	*)
	;*	segment starting at SS:0000 (and therefore not		*)
	;*	extending as far as SS:FFFF).  I have therefore		*)
	;*	switched to using a different version of this module.	*)
	;*	The present version is retained, but not used for now,	*)
	;*	in case it turns out to be the better choice with some	*)
	;*	other compiler.  In theory the compiler should not care	*)
	;*	about such things, but in practice some Modula-2	*)
	;*	systems use the stack segment to hold some non-stack	*)
	;*	information, and therefore they make assumptions about	*)
	;*	certain parts of the stack segment being accessible.	*)
	;*								*)
	...
	;*								*)
	;*	Status:		"Working" with TopSpeed 1.17 and 3.10	*)
	;*			Not tested with FST			*)
	;*								*)
	;*	NOTE: although this version has tested OK with TopSpeed	*)
	;*	it cannot be recommended, at least for TopSpeed 3.10,	*)
	;*	because of a conflict with the TopSpeed floating point	*)
	;*	library functions.  I'm not yet sure whether this	*)
	;*	conflict also exists with TopSpeed 1.17.		*)
	;*								*)
	;*	NOTE: This version does not yet incorporate some	*)
	;*	improvements from the latest InnerKernel.A.		*)
	;*	I'm now testing a better version.			*)
	;*								*)
	;****************************************************************)
...
InnerKernel$TaskInit	PROC FAR
	...
	;	StackBase: ADDRESS	doubleword	at 18[bp]
	;	StackSize: CARDINAL	word		at 16[bp]
	;	EnableInterrupts: BOOLEAN byte		at 14[bp]
	;	TaskExit: PROC		doubleword	at 10[bp]
	;	StartAddress: PROC	doubleword	at 6[bp]
        ...
        mov dx, 14[bp]		; pick up EnableInterrupts (one byte,
	...
	mov di, 0FFC8H		;*    in ES:DI for now		*)
